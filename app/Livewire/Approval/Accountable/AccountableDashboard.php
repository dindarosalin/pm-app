...
class AccountableDashboard extends Component
{
    ...
    public function getAllApproval()
    {
        ...
        if ($this->auth == $finance) {
            ...
            $this->projects = ApprovalProjectProcurement::getByAccountable($this->auth)->map(function ($item) {
                $item->subject_name = $item->project_name ?? '-';
                return $item;
            });
        } else {
            $this->permissions = ApprovalPermissions::getByAccountable($this->auth)->map(function ($item) {
                $item->subject_name = $item->subject_name ?? ($item->subject ?? '-');
                return $item;
            });

            $this->absences = ApprovalAbsences::getByAccountable($this->auth)->map(function ($item) {
                $item->subject_name = $item->subject_name ?? ($item->subject ?? '-');
                return $item;
            });
        }
